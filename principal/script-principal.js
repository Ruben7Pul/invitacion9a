console.log('🚀 principal/script-principal.js (versión con pausa de recursos en collage)');
...
function initContadorCircular(config) {
  ...
  // Exponer funciones para pausar/reanudar el contador
  window._contadorInterval = intervalId;
  window._contadorUpdate = actualizarContador;
  window._contadorIntervalMs = intervalo;
  window._contadorStop = function() {
    if (window._contadorInterval) {
      clearInterval(window._contadorInterval);
      window._contadorInterval = null;
    }
  };
  window._contadorResume = function() {
    if (window._contadorUpdate && window._contadorIntervalMs) {
      window._contadorInterval = setInterval(window._contadorUpdate, window._contadorIntervalMs);
    }
  };
}
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  function pausarRecursos() {
    ...
    if (window._contadorStop) window._contadorStop();
    // También se pueden pausar otros videos si existen
  }
  ...
  function reanudarRecursos() {
    ...
    if (window._contadorResume) window._contadorResume();
  }
